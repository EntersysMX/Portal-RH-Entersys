...
import { isDashboardWidgetEnabled, isQuickActionEnabled } from '@/modules/contributions';
...
export default function Dashboard() {
  ...
  // Subscribe to manifest for reactivity when modules toggle
  useModuleStore((s) => s.manifest);
  ...
  const showWidget = isDashboardWidgetEnabled;
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="card border-t-[3px] border-t-violet-500">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {[
              { id: 'nuevo-empleado', label: 'Nuevo Empleado', href: '/employees?action=new', color: 'bg-blue-50 text-blue-700 hover:bg-blue-100' },
              { id: 'nueva-vacante', label: 'Nueva Vacante', href: '/recruitment?action=new', color: 'bg-purple-50 text-purple-700 hover:bg-purple-100' },
              { id: 'registrar-asistencia', label: 'Registrar Asistencia', href: '/attendance', color: 'bg-green-50 text-green-700 hover:bg-green-100' },
              { id: 'procesar-nomina', label: 'Procesar Nómina', href: '/payroll', color: 'bg-orange-50 text-orange-700 hover:bg-orange-100' },
            ]
              .filter((action) => isQuickActionEnabled(action.id))
              .map((action) => (
                <a
                  key={action.label}
                  href={action.href}
            ...
            {isQuickActionEnabled('descargar-nomina') && (
              <button
                onClick={() => {
                  try {
